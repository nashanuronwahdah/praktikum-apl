...
void inisialisasiDataDummy() {
    pengguna[0] = {"budi", "123", "user"};
    pengguna[1] = {"andi", "123", "user"};
    jumlahPengguna = 2;

    wilayah[0].nama = "Bengkuring";
    wilayah[0].baris = 3; wilayah[0].kolom = 3;
    wilayah[0].totalSlot = 9;
    wilayah[0].hargaPerMakam = 750000;
    for (int i = 0; i < 9; i++) wilayah[0].slot[i] = {i + 1, 750000, false, "", "", ""};
    
    wilayah[1].nama = "Sempaja";
    wilayah[1].baris = 2; wilayah[1].kolom = 5;
    wilayah[1].totalSlot = 10;
    wilayah[1].hargaPerMakam = 1000000;
    for (int i = 0; i < 10; i++) wilayah[1].slot[i] = {i + 1, 1000000, false, "", "", ""};

    wilayah[2].nama = "LoaJanan";
    wilayah[2].baris = 2; wilayah[2].kolom = 2;
    wilayah[2].totalSlot = 4;
    wilayah[2].hargaPerMakam = 500000;
    for (int i = 0; i < 4; i++) wilayah[2].slot[i] = {i + 1, 500000, false, "", "", ""};

    wilayah[3].nama = "Palaran";
    wilayah[3].baris = 1; wilayah[3].kolom = 5;
    wilayah[3].totalSlot = 5;
    wilayah[3].hargaPerMakam = 300000;
    for (int i = 0; i < 5; i++) wilayah[3].slot[i] = {i + 1, 300000, false, "", "", ""};
    
    jumlahWilayah = 4;

    tandaiSlotDenganPointer(&wilayah[0].slot[4], "Fulan", "Bapak Fulan", "Senin, 01-01-2024");
    tandaiSlotDenganPointer(&wilayah[1].slot[9], "Joko", "Supriadi", "Rabu, 15-05-2025");
    tandaiSlotDenganPointer(&wilayah[2].slot[0], "Ahmad", "Siti", "Jumat, 10-10-2025");
    tandaiSlotDenganPointer(&wilayah[3].slot[2], "Budi", "Slamet", "Minggu, 12-12-2026");

    cout << "[SISTEM] Data dummy berhasil dimuat! Siap digunakan.\n";
}
...
void tandaiSlotTerisi() {
    ...
    cout << "Slot berhasil ditandai terisi secara manual.\n";
}
...
void sortWilayahNamaAsc() {
    for (int i = 0; i < jumlahWilayah - 1; i++) {
        for (int j = 0; j < jumlahWilayah - i - 1; j++) {
            if (wilayah[j].nama > wilayah[j+1].nama) {
                Wilayah temp = wilayah[j];
                wilayah[j] = wilayah[j+1];
                wilayah[j+1] = temp;
            }
        }
    }
    cout << "\n[!] Berhasil mengurutkan wilayah berdasarkan Nama (A-Z) menggunakan Bubble Sort.\n";
}

void sortWilayahHargaDesc() {
    for (int i = 0; i < jumlahWilayah - 1; i++) {
        int maxIdx = i;
        for (int j = i + 1; j < jumlahWilayah; j++) {
            if (wilayah[j].hargaPerMakam > wilayah[maxIdx].hargaPerMakam) {
                maxIdx = j;
            }
        }
        if (maxIdx != i) {
            Wilayah temp = wilayah[i];
            wilayah[i] = wilayah[maxIdx];
            wilayah[maxIdx] = temp;
        }
    }
    cout << "\n[!] Berhasil mengurutkan wilayah berdasarkan Harga (Termahal ke Termurah) menggunakan Selection Sort.\n";
}

void sortWilayahSlotAsc() {
    for (int i = 1; i < jumlahWilayah; i++) {
        Wilayah key = wilayah[i];
        int j = i - 1;
        while (j >= 0 && wilayah[j].totalSlot > key.totalSlot) {
            wilayah[j + 1] = wilayah[j];
            j = j - 1;
        }
        wilayah[j + 1] = key;
    }
    cout << "\n[!] Berhasil mengurutkan wilayah berdasarkan Total Slot (Sedikit ke Banyak) menggunakan Insertion Sort.\n";
}

void menuSortingAdmin() {
    if (jumlahWilayah < 2) {
        cout << "Data wilayah kurang dari 2, tidak perlu diurutkan.\n";
        return;
    }
    int pilihan;
    cout << "\n=== MENU SORTING WILAYAH ===\n";
    cout << "1. Urutkan berdasarkan Nama (A-Z) [Bubble Sort]\n";
    cout << "2. Urutkan berdasarkan Harga (Tertinggi ke Terendah) [Selection Sort]\n";
    cout << "3. Urutkan berdasarkan Kapasitas Slot (Terkecil ke Terbesar) [Insertion Sort]\n";
    cout << "Pilihan: ";
    cin >> pilihan;

    switch (pilihan) {
        case 1: sortWilayahNamaAsc(); break;
        case 2: sortWilayahHargaDesc(); break;
        case 3: sortWilayahSlotAsc(); break;
...
void menuAdmin() {
    ...
    do {
        ...
        cout << "5. Urutkan Data Wilayah (Sorting)\n";
...
void pesanMakam() {
    ...
    while (true) {
        ...
        if (konfirmasiPembayaran == "0") continue;
        ...
        if (tanggal < 1 || tanggal > 31) { cout << "Tanggal tidak valid.\n"; continue; }
        if (bulan < 1 || bulan > 12) { cout << "Bulan tidak valid.\n"; continue; }
        if (tahun <= 0) { cout << "Tahun tidak valid.\n"; continue; }
        ...
        cout << "\nPemesanan berhasil! Slot " << nomorSlot << " di wilayah " << wilayah[indeksWilayah].nama << " telah dipesan.\n";
        return;
    }
}
